...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (!empty($fullname) && !empty($password) && !empty($email) && !empty($bday) && !empty($course) && !empty($absent)) {
        $query = "select * from users where email = '$email' limit 1";
        $result = mysqli_query($conn, $query);
        if ($result->num_rows > 0) {
            echo "Email already exists. Please use a different email.";
        } else {
            $query = "INSERT into users (absent, name, birthday, course, email, password) 
            values ('$absent', '$fullname', '$bday', '$course', '$email', '$password');";
            ...
            <div class="fieldDiv row-span-4">
                <form method="post" class="m-auto">
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        <input type="number" name="absent" required placeholder="Absent" autocapitalize="on" class="border border-neutral-300 rounded-md text-black mb-2 px-4 py-2 block w-full justify-center bg-white bg-opacity-50">
                    </div>
